...
class CNNEncoder(nn.Module):
    """
    CNN encoder for image feature extraction before transformer.
    Extracts local features and reduces spatial dimensions.
    """
    def __init__(self, input_channels=1, hidden_size=256, input_height=28, input_width=28):
        super().__init__()
        ...
        # After 2 max pools: height/4 * width/4 * 128
        self.flattened_size = 128 * (input_height // 4) * (input_width // 4)
        
        # Projection to transformer hidden size
        self.projection = nn.Linear(self.flattened_size, hidden_size)
        
    def forward(self, x):
        """
        Input: (batch_size, seq_len, input_height, input_width)
        Output: (batch_size, seq_len, hidden_size)
        """
        batch_size, seq_len, height, width = x.shape
        
        # Reshape for CNN: (batch_size * seq_len, 1, height, width)
        x_reshaped = x.view(batch_size * seq_len, 1, height, width)
        
        # Apply CNN
        cnn_features = self.cnn(x_reshaped)
        
        # Flatten: (batch_size * seq_len, flattened_size)
...
class HybridCNNTransformer(nn.Module):
    def __init__(self, config, device):
        ...
        # Optional: Original linear embedding (if you want to keep it as fallback)
    ...
    def forward(self, x, y, y_output_mask, src_key_padding_mask, tgt_key_padding_mask):
        """
        x: input sequence of images (batch_size, seq_len, feature_dim)
        y: target sequence (batch_size, seq_len)
        """
        batch_size, seq_len, feature_dim = x.shape
        
        # Reshape x for CNN: assuming each feature is flattened image
        # For MNIST: feature_dim = 784 (28x28)
        if feature_dim == 784:  # Full MNIST image
            # Reshape to (batch_size, seq_len, 28, 28)
            x_images = x.view(batch_size, seq_len, 28, 28)
            # Use CNN encoder
            x = self.cnn_encoder(x_images)
        else:
            # For sliding window features, use linear embedding
        ...
        # Target embedding
        y_embed = self.y_embedding(y) * math.sqrt(self.hidden_size)
        y_embed = self.positional_encoding_layer(y_embed)
